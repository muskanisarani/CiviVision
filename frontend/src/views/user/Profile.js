...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    // Fetch dynamic database statistics
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.complaints) {
            ...
            const filedCount = userComplaints.length;
            const resolvedCount = userComplaints.filter(c => c.status === 'Resolved').length;
            const feedbackCount = userComplaints.filter(c => c.rating !== null).length;
  ...
  return (
    <div style={styles.body}>
      <div style={styles.topHeader}>
        ...
        <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '800', letterSpacing: '-0.25px', color: 'var(--text-primary)' }}>My Citizen Profile</h3>
      </div>
        ...
        <div className="row g-4 mb-4">
          ...
          <div className="col-lg-6">
            <div style={styles.passCard}>
              <div style={{ display: 'flex', justify_content: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                <div>
